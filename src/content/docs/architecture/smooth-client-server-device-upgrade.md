...
- **发布顺序**　先发布兼容新旧契约的后端。确认新后端稳定后，再升级设备或发布客户端。
- **兼容方向**　新后端承接旧端，新客户端和新固件不必适配旧后端。
- **接口版本**　增加可选字段时沿用原契约。字段语义、状态机或业务规则发生破坏性变化时，再并行保留 v1 和 v2。
- **设备判断**　多种硬件按能力协商，固件版本只用于排障、分组和选择 OTA 包。
- **状态处理**　状态同步与命令执行分开。幂等键、规则版本和状态版本都要进入持久化记录。
- **路径退役**　查看流量、版本覆盖率和离线队列，不能只看代码里还有没有调用者。
...
多种硬件长期共存时，优先选能力驱动的服务端编排。旧协议留在 Adapter 或边缘网关，新功能由能力交集放行，OTA 逐步缩小版本分布。最低能力交集可以用于某一次群组操作，不宜把整个产品永久锁在最老设备上。
...
等新后端稳定运行后，再升级设备或发布客户端。兼容关系因此主要保持单向，新后端承接旧端，新端只连接新后端。系统不用维护反向兼容分支，协议、测试和回滚成本都会小很多。这里的稳定至少包括流量已切换、关键指标正常、所有区域和滚动实例完成升级。新端启用后，后端回滚也不能越过这条协议边界。
