...
普通任务可能就此结束，需要人发现遗漏，再要求它继续。下面的实现额外加入一个文件检查器：它读取两个页面，发现关于页还是旧年份，就把这个失败项送回 Agent，让它再做一轮。

自动续跑和检查结果是两项不同的能力。程序可以持续催 Agent 工作，却仍然相信它自己报告的“已经完成”。这里演示的是加了文件检查器的做法，不是所有 Goal 功能的默认行为。
...
“一直做，直到完成”这句提示可以影响 Agent 的行为。Agent 结束本轮后，控制程序再判断是否启动下一轮。普通 Agent 也可以有长时间执行、计划和上下文压缩，这些都不专属于目标模式。[OpenCode 的普通执行循环](https://opencode.ai/docs/agents/#max-steps)
...
不要锁着数据库等待几分钟的模型响应。用短事务领取任务，在事务外执行，再用短事务检查执行权并保存结果。

```text
领取任务    检查状态与版本，取得租约，预留预算
执行工作    调用模型和工具，生成候选文件，运行检查
保存结果    结算用量，核对版本与执行权，再更新目标状态
```

旧进程已经失去执行权，即使后来交回结果，也不能再修改任务状态。但它调用模型花的钱仍然要记录。保存费用可以用独立的事务处理，避免拒绝旧进程的结果时，把费用记录也撤销了。
