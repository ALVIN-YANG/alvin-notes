...
**选择多智能体架构，先看任务需要怎样交换信息。步骤紧密相连，就先让一个 Agent 连续做；几项工作能各自完成，就独立并行；需要统一分工和纠错，就设置总管；执行者需要互相交换发现，就开放同伴通信；既要统一控制又要局部协作，才考虑混合架构。**

这五种方式分别是 SAS、Independent、Centralized、Decentralized 和 Hybrid。它们没有从低级到高级的排名。多出来的 Agent 可能扩大调查范围，也可能把时间和 token 花在重复劳动、转述和争论上。是否值得拆，最终要与同条件下的单 Agent 比较。
...
这是教学示例，不是某次真实迁移记录。为了对比，暂时把调查分成三块：A 查兼容性，B 测性能，C 算成本。字母表示执行者，不代表必须使用不同模型。
...
这个例子里，前一步的发现直接决定后一步怎么做，保留完整过程很有用。难点是资料多起来以后，一个 Agent 可能顾不过来。应先确认是缺资料、工具不好用，还是确实需要拆出独立调查，不要仅凭“任务很长”就换架构。
...
其他任务又呈现不同结果。Finance-Agent 中，Centralized 的平均表现从 SAS 的 34.9% 提高到 63.1%；BrowseComp-Plus 中，Decentralized 从 31.8% 提高到 34.7%。这支持按任务结构选型，不支持给五种架构排一个永久榜单。

“Independent 收益最低”也要限定实验范围。它缺少中途协作，未必适合这个迁移例子；但如果只是各自查清三份互不相关的资料，增加讨论不一定更好。模型、工具、预算和汇总方法变化以后，都需要重新评测。

## 真正选型时，先问这几个问题

下面是基于上述机制的工程判断，不是论文给出的通用排名。

| 先检查什么 | 优先尝试什么 |
| --- | --- |
| 下一步必须等上一步的实际结果，而且过程不长 | SAS，保留连续的任务过程 |
| 各项调查可以独立完成，结果有明确的合并方法 | Independent，单独设计必要的结果校验 |
| 中途经常需要重新分工、补查和统一处理冲突 | Centralized，让总管负责调整 |
| 执行者之间需要直接交换发现，不必每件事都统一裁决 | Decentralized，限制讨论轮次并明确结束规则 |
| 总体目标必须统一，部分执行者又需要频繁对齐 | Hybrid，只开放必要的同伴通信 |

拿迁移评估来说，可以先跑 SAS。如果日志显示，兼容性、性能和成本调查挤在一个上下文里，导致反复丢失证据，再试 Centralized。只有实际出现“A、B 不断通过总管转述细节”的问题，才有理由增加局部通信，而不是一开始就开放所有人互聊。
...
先实现 SAS 基线，再挑一个有明确理由的多智能体方案对照，不必一次做全五套。已有 SDK 能创建独立上下文、调用工具并返回结果时，可以先复用它；复杂的暂停恢复和分支再交给编排框架。框架名字不决定谁和谁通信。
...
对比时，固定问题、资料版本、可用工具和验收标准，并给两种方案相同的总费用上限。把总管、子 Agent、汇总和重试全部计入费用。遇到结果波动，要重复运行，不能只拿一次成功截图作结论。

迁移评估可以检查四件事：关键不兼容项有没有漏掉；性能结论是否基于正确的改写版本；成本是否覆盖必要改造；证据不足时是否明确说明。同时记录总耗时、实际费用和人工补查次数。还应故意让一个子任务超时、返回互相矛盾的证据，检查系统如何收尾。

如果拆分没有减少遗漏、缩短等待或降低维护负担，就保留 SAS。架构的价值在于让任务更容易完成和验证，不在于让组织图更像一家公司。
